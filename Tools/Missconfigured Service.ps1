...
$results = Get-WmiObject Win32_Service | ForEach-Object {
    ...
    if ($SDDL) {
        ...
        # Vulnerable rights: CC, DC, WD, WO, WP, DT, FA, SW, RP
        $dangerous = 'CC|DC|WD|WO|WP|DT|FA|SW|RP'

        $ServiceWeakDACL = (
            $SDDL -match "BU.*\($dangerous" -or
            $SDDL -match "IU.*\($dangerous" -or
            $SDDL -match "SU.*\($dangerous" -or
            $SDDL -match "AU.*\($dangerous" -or
            $SDDL -match "WD.*\($dangerous" -or
            $SDDL -match "$($env:USERNAME).*($dangerous)"
        )
    }
...
# $results | Out-File ".\svc_vulnerable.txt" -Encoding UTF8
